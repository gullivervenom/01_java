...
public class Application {
    public static void main(String[] args) {
        ...
        *   */
        ...
        System.out.println("ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ너구리 생성 ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ");
        너구리 raccoon = new 너구리();
        raccoon.먹기();
        raccoon.깨물기();
        raccoon.뛰기();
        ...
        System.out.println("ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ개구리 생성 ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ");
        개구리 frog = new 개구리();
        frog.먹기();
        frog.낼름공격();
        frog.뛰기();
...
//        개구리 f1 = new 동물();
        ...
        *   런타임 시 실제 객체가 가진 오버라이딩 된 메소드(개구리, 너구리)로 바인딩이 바뀌어
        *   동작하는 것을 의미한다. */
        System.out.println("ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ동적 바인딩 확인 ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ");
        a1.낄낄거리기();
        a2.낄낄거리기();
...
//        a1.낼름공격();
        ...
        /* index. 3. 타입 형변환 확인 */
        System.out.println("ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ클래스 타입 형변환 ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ");
        ((개구리)a1).낼름공격();
        ((너구리)a2).깨물기();

        // 주의사항 : 타입 형변환을 잘못하게 되면
        // 컴파일 시에는 문제가 없지만 런타임시 ClassCastException 오류 발생
//        ((너구리)a1).깨물기();

        /* index. 4. instanceof 연산자로 확인 */
        System.out.println("ㅡㅡㅡㅡㅡㅡㅡinstanceofㅡㅡㅡㅡㅡㅡㅡㅡㅡ");
