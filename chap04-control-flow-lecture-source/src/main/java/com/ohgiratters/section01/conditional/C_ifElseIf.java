...
public class C_ifElseIf {
    ...
    /* title. if, if-esle, if-else-if 문 단독 사용에 대한 흐름을 이해하고 적용할 수 있다. */
    ...
    *   조건식 1이 false 이면서 조건식 2가 참이면 else if {} 블럭 실행
    *   조건식 1과 2가 모두 거짓(false) 이면 else{} 블럭을 실행한다. */
    ...
    public void improvetNestedIfElseIfStatement(){
        /* comment. 중첩된 if-else-if 구문*/
        /* index. 1. 문제
        *   ohgiraffers. 대학교의 조**교수는 학생들의 성적을 입력해서
        *   학점 등급을 매기는 채점방식을 사용하고 있다.
        *   학생의 이름과 점수를 입력 받아
        *   90점 이상이면 'A', 80 점 이상이면 'b', 70점 이상이면 'C', 60 점 이상이면 'D',
        *   60 점 미만이면 'F' 학점을 줄 것이다.
