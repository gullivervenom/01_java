...
//            et.checkEnoughMoney(3000,5000);
            ...
            /* title.
                주의사항
            *   catch 블럭은 위에서 부터 아래로 자신의 타입과 맞는 경우 동작을 하게 되는데
                상위 타입 (NegativeException) 이 catch 블럭 상단에 위치하게 된다면
                하단에 작성한 하위 타입(MoneyNegativeException) 은
                절대 도달할 수없는 코드가 되기 때문에 컴파이르 에러를 발생시킨다. */
            // 예외 상황 별 catch 블럭을 따로 작성해서 처리할 수 있다.
        } catch (PriceNegativeException e) {
            System.out.println("가격이 음수 발생");
            System.out.println(e.getMessage());

        }
        catch (MoneyNegativeException e) {
            System.out.println("돈이 음수인 상황 발생!!!");
            System.out.println(e.getMessage());
        }
            ...
            System.out.println("finally 블럭 예외 없이 동작 확인...");
